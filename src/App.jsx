...
const PRACTICE_TEMPLATES = [
  { id: "tmpl1", name: "Early Season — Fundamentals & Team Building", duration: 120, focus: "Fundamentals",
    ...
    phases: [
      { name: "Warm-Up", type: "together", time: "15 min", drills: [
        ...
        { ...D("m1"), assignedCoach: "Head Coach", notes: "Topic: 'What does being a Pirate mean? We support each other.'" },
      ]},
      { name: "Station Rotations", type: "stations", time: "45 min (3 groups × 15 min each)", drills: [
        { ...D("t1"), assignedCoach: "Shari", station: "A" },
        { ...D("f1"), assignedCoach: "Ken", station: "B" },
        { ...D("h1"), assignedCoach: "Head Coach", station: "C" },
      ]},
  ...
  { id: "tmpl2", name: "Mid Season — Game Situations & Refining", duration: 120, focus: "Game Situations",
    ...
    phases: [
      ...
      { name: "Full Team — Game Situations", type: "together", time: "35 min", drills: [
        { ...D("g1"), assignedCoach: "All", notes: "Call different scenarios each round" },
        { ...D("g2"), assignedCoach: "Ken" },
      ]},
      { name: "Huddle", type: "together", time: "5 min", drills: [
        { ...D("m1"), assignedCoach: "Head Coach", notes: "Topic: What did we learn from last game?" },
      ]},
    ]},
  { id: "tmpl3", name: "Pre-Game — Light Work & Mental Prep", duration: 120, focus: "Pre-Game",
    description: "Game day or day before. Light reps, build confidence. No heavy drilling.",
    phases: [
      ...
      { name: "Defensive Review", type: "together", time: "25 min", drills: [
        ...
        { ...D("f4"), assignedCoach: "Ken", notes: "Fly ball tracking — outfielders especially" },
      ]},
      { name: "Mental Prep", type: "together", time: "10 min", drills: [
        { ...D("m1"), assignedCoach: "Head Coach", notes: "Talk about opponent, focus cues, positive energy. 'Trust your training.'" },
      ]},
      { name: "Fun Finish", type: "together", time: "15 min", drills: [
        { ...D("h9"), assignedCoach: "Any", notes: "Points-Based Hitting — end on a high note" },
      ]},
    ]},
  { id: "tmpl4", name: "Hitting-Focused Day", duration: 120, focus: "Hitting",
    ...
    phases: [
      ...
      { name: "Hitting Station Rotations", type: "stations", time: "45 min (3 groups × 15 min each)", drills: [
        { ...D("h6"), assignedCoach: "Head Coach", station: "A", notes: "Jugs machine — vary speeds each round" },
        { ...D("h3"), assignedCoach: "Ken", station: "B", notes: "Inside/outside tee — move tee between reps" },
        { ...D("h5"), assignedCoach: "Shari", station: "C", notes: "Front toss — focus on timing" },
      ]},
      ...
      { name: "Cool Down", type: "together", time: "5 min", drills: [
        { ...D("m1"), assignedCoach: "Head Coach", notes: "What felt good today? What do we work on next?" },
      ]},
    ]},
];
...
const PracticePlanner = () => {
  const [list, setList] = useState([]); const [show, setShow] = useState(false); const [form, setForm] = useState({ id:"", date:"", time:"", duration:120, focus:"", drills:[], notes:"" }); const [ed, setEd] = useState(null); const [df, setDf] = useState("all"); const [exp, setExp] = useState(null);
  const [expandedDrill, setExpandedDrill] = useState(null);
  const [showTemplates, setShowTemplates] = useState(false);

  useEffect(() => { loadStore(STORAGE_KEYS.PRACTICES, []).then(setList); }, []); useEffect(() => { saveStore(STORAGE_KEYS.PRACTICES, list); }, [list]);
  const cats = ["all", ...new Set(DRILL_LIBRARY.map(d => d.category))]; const fd = df==="all"?DRILL_LIBRARY:DRILL_LIBRARY.filter(d => d.category===df); const tt = form.drills.reduce((s,d)=>s+d.duration,0);
  const save = () => { if (ed) setList(p => p.map(x => x.id===ed?{...form}:x)); else setList(p => [...p, {...form, id: Date.now().toString()}]); setShow(false); setEd(null); };
  ...
  return <div>
    ...
    <Modal open={show} onClose={() => { setShow(false); setEd(null); }} title={ed?"Edit Practice":"Plan Practice"} wide>
      ...
      <div style={{ marginTop: 16 }}><div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}><SL>Practice Plan — {tt}/{form.duration}min</SL><div style={{ width: 120, height: 6, background: THEME.charcoal, borderRadius: 3 }}><div style={{ width: `${Math.min(100,(tt/form.duration)*100)}%`, height: "100%", background: tt>form.duration?THEME.red:THEME.gold, borderRadius: 3 }} /></div></div>
        {form.drills.length>0 && <div style={{ marginBottom: 12 }}>{(() => {
          let lastPhase = "";
              ...
              <div style={{ background: THEME.black, borderRadius: 6, marginBottom: 3, overflow: "hidden", borderLeft: d.station ? `3px solid ${THEME.gold}` : "3px solid transparent" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 10px" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 2 }}>
                    ...
                    <button disabled={i === form.drills.length - 1} onClick={() => { const arr = [...form.drills]; [arr[i], arr[i+1]] = [arr[i+1], arr[i]]; setForm({...form, drills: arr}); }} style={{ background: "none", border: "none", color: i === form.drills.length - 1 ? THEME.charcoal : THEME.gold, cursor: i === form.drills.length - 1 ? "default" : "pointer", fontSize: 14, padding: "2px" }}>▼</button>
    ...
    <Modal open={show} onClose={() => { setShow(false); setEd(null); setCurBatterIdx(0); }} title={ed ? "Edit Game" : "New Game"} wide>
      ...
      {mode === "score" && <div>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
          <button onClick={() => setDefenseMode("defense")} style={{ flex: 1, padding: "12px 0", fontSize: 14, fontWeight: 700, borderRadius: 8, cursor: "pointer", background: defenseMode === "defense" ? THEME.red : THEME.charcoal, color: THEME.white, border: "none", fontFamily: "'Oswald',sans-serif", textTransform: "uppercase" }}>🛡️ Defense (Top)</button>
          <button onClick={() => setDefenseMode("offense")} style={{ flex: 1, padding: "12px 0", fontSize: 14, fontWeight: 700, borderRadius: 8, cursor: "pointer", background: defenseMode === "offense" ? THEME.green : THEME.charcoal, color: THEME.white, border: "none", fontFamily: "'Oswald',sans-serif", textTransform: "uppercase" }}>⚾ Offense (Bottom)</button>
        </div>
        ...
        {defenseMode === "defense" && <div>
          ...
          {currentPitcher && (() => {
            ...
            return <Card style={{ marginBottom: 12 }}>
              ...
              <button onClick={() => {
                // Save this inning's pitch count
                setForm({
                  ...form,
                  defensiveStats: {
                    ...form.defensiveStats,
                    [currentPitcher]: {
                      ...(form.defensiveStats?.[currentPitcher] || { putouts: 0, assists: 0, errors: 0, pitches: 0 }),
                      pitches: (form.defensiveStats?.[currentPitcher]?.pitches || 0) + pitchCount.pitches
                    }
                  }
                });
                setPitchCount({ pitches: 0, strikes: 0, balls: 0 });
              }} style={{ width: "100%", padding: "10px 0", fontSize: 13, fontWeight: 700, borderRadius: 6, cursor: "pointer", background: THEME.charcoal, color: THEME.white, border: "none", fontFamily: "'Oswald',sans-serif" }}>
                End Inning (Save Pitch Count)
              </button>
            </Card>;
          })()}
  ...
  return <div style={{ minHeight: "100vh", background: THEME.black, color: THEME.white, fontFamily: "'Source Sans 3',sans-serif", padding: "0 0 40px 0" }}>
    ...
    <div style={{ background: `linear-gradient(135deg, ${THEME.black} 0%, ${THEME.blackLight} 100%)`, borderBottom: `3px solid ${THEME.gold}`, padding: "20px 24px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
        <div style={{ width: 56, height: 56, borderRadius: "50%", background: THEME.gold, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, boxShadow: `0 0 20px ${THEME.gold}40` }}>🏴‍☠️</div>
        ...
        <button onClick={() => { if (confirm("Reset roster to original 13 players?")) { setPlayers(SEED_PLAYERS); try { window.storage.set(STORAGE_KEYS.PLAYERS, JSON.stringify(SEED_PLAYERS)); } catch{} } }} style={{ marginLeft: "auto", background: "none", border: `1px solid ${THEME.charcoal}`, color: THEME.gray, padding: "6px 12px", borderRadius: 6, fontSize: 11, cursor: "pointer", fontFamily: "'Oswald',sans-serif", textTransform: "uppercase" }}>Reset Roster</button>
      </div>
