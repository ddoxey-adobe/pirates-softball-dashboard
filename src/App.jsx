...
const SchedulePanel = () => {
  ...
  const today = new Date().toISOString().split("T")[0];
  const filtered = filter === "all" ? SEASON_SCHEDULE : SEASON_SCHEDULE.filter(e => e.type === filter);
  const phases = [...new Set(SEASON_SCHEDULE.map(e => e.phase))];
  ...
  const nextEvent = getNextScheduleEvent();
  const gamesCount = SEASON_SCHEDULE.filter(e => e.type === "game").length;
  const practicesCount = SEASON_SCHEDULE.filter(e => e.type === "practice").length;
  const pastEvents = SEASON_SCHEDULE.filter(e => e.date < today).length;
  const remainingEvents = SEASON_SCHEDULE.filter(e => e.date >= today).length;
      ...
      return <div key={phase} style={{ marginBottom: 16 }}>
        ...
        {isExpanded && <div style={{ display: "grid", gap: 6 }}>
          {phaseEvents.map(ev => {
            ...
            return <Card key={ev.id} style={{
              padding: "12px 16px", opacity: isPast ? 0.5 : 1,
