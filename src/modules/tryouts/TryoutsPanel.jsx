...
const TryoutsPanel = () => {
  ...
  // The #register case is handled internally (registration mode), but we
  // check for #tryouts here so the module system knows to render us.
  useEffect(() => {
    const hash = window.location.hash;
    if (hash === '#tryouts' || hash === '#register') {
      // Module is active — no action needed, the module loader handles display.
      // The #register hash is additionally handled by the internal registration
      // mode logic below (existing behavior from the original component).
    }
  }, []);
...
// ─── MODULE EXPORT ────────────────────────────────────────────────────────
// Default export so the module loader can import via:
//   export { default } from './TryoutsPanel';
// ──────────────────────────────────────────────────────────────────────────

// Static helper: check if this module should be active based on URL hash
TryoutsPanel.shouldActivate = () => {
  const hash = window.location.hash;
  return hash === '#tryouts' || hash === '#register';
};
