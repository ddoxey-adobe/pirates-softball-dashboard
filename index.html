...
const printPracticePlan = (practice, players) => {
  try {
    const attendingPlayers = players.filter(p => practice.attendance?.[p.id]);
    const drillsByPhase = {};

    (practice.drills || []).forEach(drill => {
      const phase = drill.phase || "Other";
      if (!drillsByPhase[phase]) drillsByPhase[phase] = [];
      drillsByPhase[phase].push(drill);
    });
  ...
  printWindow.document.write(`
    <html>
    <head>
      <title>Practice Plan - ${practice.date || 'TBD'}</title>
      <style>
        @media print {
          @page { margin: 0.5in; }
        }
    ...
    <body>
      <h1>🏴‍☠️ Pirates Softball Practice Plan</h1>
      <div class="meta">
        <div class="meta-item">
          <div class="meta-label">Date:</div>
          ${practice.date ? new Date(practice.date + "T12:00:00").toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric", year: "numeric" }) : "TBD"}
        </div>
        <div class="meta-item">
        ...
        <div class="meta-item">
          ...
          ${practice.duration || "TBD"} minutes
        </div>
        <div class="meta-item">
          <div class="meta-label">Focus:</div>
          ${practice.focus || "General Practice"}
        </div>
        <div class="meta-item">
          <div class="meta-label">Expected Attendance:</div>
          ${attendingPlayers.length} players
        </div>
        <div class="meta-item">
          <div class="meta-label">Total Drills:</div>
          ${(practice.drills || []).length}
        </div>
      </div>
      ...
      ${practice.notes ? `
        <div class="section">
          ...
          <p style="font-size: 14px; line-height: 1.6;">${practice.notes}</p>
        </div>
      ` : ''}

      <div class="section">
        <div class="section-title">👥 Expected Players (${attendingPlayers.length})</div>
        <ul class="players-list">
          ${attendingPlayers.map(p => `<li>✓ ${p.name}</li>`).join('')}
        </ul>
      </div>

      ${Object.entries(drillsByPhase).map(([phase, drills]) => `
        <div class="section">
          <div class="section-title">${phase}</div>
          ${drills.map(drill => {
            const drillInfo = DRILL_LIBRARY.find(d => d.id === drill.id) || {};
            return `
              <div class="drill">
                <div class="drill-name">${drill.name}</div>
                <div class="drill-meta">
                  ⏱️ ${drill.duration} min
                  ${drill.assignedCoach ? ` | 👤 ${drill.assignedCoach}` : ''}
                  ${drill.station ? ` | 📍 Station ${drill.station}` : ''}
                </div>
                ${drillInfo.description ? `<div class="drill-desc">${drillInfo.description}</div>` : ''}
              </div>
            `;
          }).join('')}
        </div>
...
const printPracticeReport = (practice, players) => {
  try {
    const attendingPlayers = players.filter(p => practice.attendance?.[p.id]);
  ...
  printWindow.document.write(`
    <html>
    <head>
      <title>Practice Report - ${practice.date || 'No date'}</title>
      <style>
        @media print {
          @page { margin: 0.5in; }
        }
        ...
        .observation-player { font-weight: bold; color: #3498DB; }
        .players-list {
          columns: 3;
          font-size: 13px;
          list-style: none;
    ...
    <body>
      <h1>✅ Pirates Softball Practice Report</h1>
      <div class="meta">
        <div class="meta-item">
          <div class="meta-label">Date:</div>
          ${practice.date ? new Date(practice.date + "T12:00:00").toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric", year: "numeric" }) : "No date"}
        </div>
        <div class="meta-item">
          <div class="meta-label">Focus:</div>
          ${practice.focus || "General Practice"}
        ...
        <div class="meta-item">
          ...
          ${attendingPlayers.length} / ${players.length} players
        </div>
      </div>

      <div class="section">
        <div class="section-title">👥 Players Present (${attendingPlayers.length})</div>
        <ul class="players-list">
          ${attendingPlayers.map(p => `<li>✓ ${p.name}</li>`).join('')}
        </ul>
      </div>
      ...
      ${practice.drillsRun ? `
        <div class="section">
          <div class="section-title">🏃 Drills Completed</div>
          <p style="font-size: 14px; line-height: 1.6;">${practice.drillsRun}</p>
        </div>
      ` : ''}

      ${practice.drillTracking && Object.keys(practice.drillTracking).length > 0 ? `
        <div class="section">
          <div class="section-title">📊 Drill Metrics</div>
          ${Object.entries(practice.drillTracking).map(([drillId, data]) => {
            ...
            return `
              <div class="drill-results">
                <div class="drill-name">${drill.name}</div>
                ${config.perPlayer ? `
                  ${players.filter(p => practice.attendance?.[p.id] && data[p.id]).map(p => {
                ...
                ` : `
                  <div class="player-result">
                    <div class="player-name">Team Result:</div>
                    <div>${data} consecutive outs</div>
                  </div>
                `}
              </div>
            `;
      ...
      ${practice.observations && Object.values(practice.observations).some(o => o) ? `
        <div class="section">
          <div class="section-title">👀 Player Observations</div>
          ${players.filter(p => practice.attendance?.[p.id] && practice.observations[p.id]).map(p => `
            <div class="observation">
              <span class="observation-player">${p.name}:</span>
              ${practice.observations[p.id]}
            </div>
          `).join('')}
        </div>
      ` : ''}
      ...
      ${practice.coachNotes ? `
        <div class="section">
          ...
          <p style="font-size: 14px; line-height: 1.6;">${practice.coachNotes}</p>
        </div>
      ` : ''}
              ...
              {planned.map(p => (
                <Card key={p.id} style={{ padding: 14, cursor: "pointer", border: `1px solid ${THEME.gold}40` }} onClick={() => setExp(exp === p.id ? null : p.id)}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <div style={{ display: "flex", gap: 4 }}>
                      <Button small onClick={e => { e.stopPropagation(); printPracticePlan(p, players); }}>🖨️ Print</Button>
              ...
              {completed.map(l => (
                <Card key={l.id} style={{ padding: 14, cursor: "pointer" }} onClick={() => setExp(exp === l.id ? null : l.id)}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <div style={{ display: "flex", gap: 4 }}>
                      <Button small onClick={e => { e.stopPropagation(); printPracticeReport(l, players); }}>🖨️ Print</Button>
    ...
    <div style={{ background: `linear-gradient(135deg, ${THEME.black} 0%, ${THEME.blackLight} 100%)`, borderBottom: `3px solid ${THEME.gold}`, padding: "20px 24px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
        ...
        <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
          <button onClick={() => { if (confirm("Load test practice and game data?")) { try { window.storage.set("pirates-practices-unified-2026v1", JSON.stringify(SEED_PRACTICES)); window.storage.set(STORAGE_KEYS.GAMELOGS, JSON.stringify(SEED_GAMELOGS)); alert("Test data loaded! Go to Reports or Practice/Game tabs to see it."); } catch{} } }} style={{ background: THEME.gold, border: "none", color: THEME.black, padding: "6px 12px", borderRadius: 6, fontSize: 11, cursor: "pointer", fontFamily: "'Oswald',sans-serif", textTransform: "uppercase", fontWeight: 700 }}>Load Test Data</button>
